DROP PROCEDURE "AB_VENDOR_1";
CREATE PROCEDURE "AB_VENDOR_1"
(
 IN DateFrom DATE,
 IN DateTo DATE,
...
CASE WHEN B."ObjType"='18' AND B."CurSource"='L' AND A."BASE_REF"=B."DocNum" THEN B."DocTotal"
	...
	--B."PaidToDate"
CASE WHEN B."ObjType"='18' AND B."CurSource"='L' AND A."BASE_REF"=B."DocNum" THEN B."PaidToDate" 
     WHEN B."ObjType"='18' AND B."CurSource"='C' AND A."BASE_REF"=B."DocNum" THEN B."PaidFC"
	 WHEN C."ObjType"='19' AND B."CurSource"='L' AND A."BASE_REF"=C."DocNum" THEN C."PaidToDate"
	 WHEN C."ObjType"='19' AND C."CurSource"='C' AND A."BASE_REF"=C."DocNum" THEN B."PaidFC"
	 WHEN D."ObjType"='204' AND B."CurSource"='L' AND A."BASE_REF"=D."DocNum" THEN D."PaidToDate"
	 WHEN D."ObjType"='204' AND D."CurSource"='C' AND A."BASE_REF"=D."DocNum" THEN B."PaidFC"
...
CASE WHEN B."ObjType"='18' AND B."CurSource"='L' THEN B."DocTotal"
	...
	ELSE 0 END AS "DocTotal",
--	B."PaidToDate" B."PaidSys" 
CASE WHEN B."ObjType"='18' AND B."CurSource"='L'  THEN B."PaidToDate" 
     WHEN B."ObjType"='18' AND B."CurSource"='C'  THEN B."PaidFC"
		ELSE 0 END AS "AppliedAmount",
...
AND B."DocType" = 'S'
union all
Select T0."CardCode",T0."CardName",'' "VENDor_ref",
"DocDate" AS "DocDate"
,"DocDueDate" AS "DocDueDate"
,'Advance'AS "Document_Type"
,'' AS "PoNumber",
-("DocTotalFC") AS "DocTotal"
,-("OpenBalFc") AS "AppliedAmount"
,T2."Currency" as "DocCur",T0."DocNum" as "TransNo"
from OVPM T0 LEFT JOIN VPM2 T1 ON T0."DocNum" = T1."DocNum" 
inner join OCRD T2 on T2."CardCode" = T0."CardCode"
Where "Canceled" <> 'Y'
--and "CardCode"='REF_INDIA-INR'
and COALESCE(NULLIF(T1."SumApplied",0),0) = 0
)P 
WHERE P."DocDate" between :FromDate AND :ToDate order by P."Document_Type";
